...
import { GradeMobileCard } from "./GradeMobileCard";
...
interface GradeTableProps {
  ...
  onOpenAiDialog: (state: any) => void;
}

export function GradeTable({ filteredStudents, getStudentScore, onGradeChange, onOpenAiDialog }: GradeTableProps) {
  return (
    <div className="w-full">
      <div className="block md:hidden space-y-3.5">
        {filteredStudents.map((s, idx) => {
          ...
          return (
            <GradeMobileCard key={s.id || idx} s={s} idx={idx} formatif={formatif} sts={sts} sas={sas} finalGrade={finalGrade} onGradeChange={onGradeChange} onOpenAiDialog={onOpenAiDialog} />
          );
        })}
      ...
      <div className="hidden md:block">
        <Table>
          ...
          <TableBody>
              ...
              return (
                <TableRow key={s.id || idx} className="hover:bg-white/40 border-slate-100 transition-colors">
                  ...
                  <TableCell className="text-center">
                    <Input type="number" min={0} max={100} value={formatif} onChange={e => onGradeChange(s.id, "Formatif", parseInt(e.target.value))} className="w-20 text-center h-8 text-xs mx-auto font-black rounded-lg focus:ring-primary/20" />
                  </TableCell>
                  <TableCell className="text-center">
                    <Input type="number" min={0} max={100} value={sts} onChange={e => onGradeChange(s.id, "STS", parseInt(e.target.value))} className="w-20 text-center h-8 text-xs mx-auto font-black rounded-lg focus:ring-primary/20" />
                  </TableCell>
                  <TableCell className="text-center">
                    <Input type="number" min={0} max={100} value={sas} onChange={e => onGradeChange(s.id, "SAS", parseInt(e.target.value))} className="w-20 text-center h-8 text-xs mx-auto font-black rounded-lg focus:ring-primary/20" />
                  </TableCell>
                  <TableCell className="text-center">
                    <div className="flex flex-col items-center gap-1 justify-center">
                      ...
                      <Button size="sm" variant="ghost" onClick={() => onOpenAiDialog({ open: true, studentName: s.name, studentClass: s.classId, score: finalGrade })} className="h-6 px-2 text-[9px] font-black text-primary hover:text-primary-dark gap-1 hover:bg-cyan-50/50 rounded-md">
            ...
            {filteredStudents.length === 0 && (
              <TableRow>
                <TableCell colSpan={7} className="text-center text-slate-400 py-8 text-xs font-semibold">Tidak ada data siswa ditemukan untuk filter ini</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
