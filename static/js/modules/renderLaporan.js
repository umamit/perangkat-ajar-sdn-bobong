...
export function renderLaporan() {
    ...
    const avgFormatif = Math.round(appData.students.reduce((acc, curr) => acc + (curr.scoreFormatif ?? 0), 0) / total);
    const avgSumatif = Math.round(appData.students.reduce((acc, curr) => acc + (curr.scoreSumatif ?? 0), 0) / total);
